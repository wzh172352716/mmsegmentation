...
class Distortion2(BaseTransform):
    ...
    def create_args(self, width, height):
        ...
        for x in range(width):
            for y in range(height):
                ...
                if diff_center_y > 0:
                    p = Point(x, y, x, int(center_y - min(1, self.compression_rate / ((1 / pixel_size) * diff_center_y + 1)) * diff_center_y))
                else:
                    p = Point(x, y, x,
                              int(center_y + min(1, self.compression_rate / ((1 / pixel_size) * abs(diff_center_y) + 1)) * abs(diff_center_y)))
                # print(p)
                flex_x[p[3], x] = x  # p1[0]
                flex_y[p[3], x] = y
