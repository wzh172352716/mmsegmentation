...
from .zegclip_untils import tokenize
import numpy as np
import tqdm
...
class ZegCLIP(EncoderDecoder):
    ...
    def _decode_head_forward_train(self, inputs, data_samples):
        ...
        training."""
        #print("_decode_head_forward_train")
        gt_semantic_seg = [
            data_sample.gt_sem_seg.data for data_sample in data_samples
        ]
        gt_semantic_seg = torch.stack(gt_semantic_seg, dim=0)
    ...
    def extract_vis_feat(self, img):
        """Extract features from images."""
        #print("extract_vis_feat")
    ...
    def extract_feat(self, img):
        #print("extract_feat")
        with torch.no_grad():
            visual_feat = self.extract_vis_feat(img)
    ...
    # TODO refactor
    #def slide_inference(self, img, img_meta, rescale=True):
    ...
    def slide_inference(self, inputs: Tensor,
        ...
        out_channels = len(self.both_class)
        h_grids = max(h_img - h_crop + h_stride - 1, 0) // h_stride + 1
        w_grids = max(w_img - w_crop + w_stride - 1, 0) // w_stride + 1
