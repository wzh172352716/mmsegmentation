...
from mmcv.transforms import Normalize, Pad, RandomFlip, Resize, RandomResize
...
class SeqResize(Resize):
    ...
    Please refer to `mmdet.datasets.TRANSFORMS.transforms.py:Resize` for
    detailed docstring.
...
@TRANSFORMS.register_module()
class SeqRandomResize(RandomResize):
    """Resize images.
    ...
    def __call__(self, results):
        ...
        Returns:
            ...
            are added into result dict.
        """
        outs, scale = [], self._random_scale()
        self.resize.scale = scale
        for i, _results in enumerate(results):
            _results['scale'] = scale
            _results = self.resize(_results)
            outs.append(_results)
        return outs
...
class SeqNormalize(Normalize):
    ...
    Please refer to `mmdet.datasets.TRANSFORMS.transforms.py:Normalize` for
    detailed docstring.
    """
...
class SeqRandomFlip(RandomFlip):
    ...
    Please refer to `mmdet.datasets.TRANSFORMS.transforms.py:RandomFlip` for
    detailed docstring.
    ...
    def __call__(self, results):
        ...
        if self.share_params:
            ...
            if isinstance(self.prob, list):
                non_flip_ratio = 1 - sum(self.prob)
                flip_ratio_list = self.prob + [non_flip_ratio]
            else:
                non_flip_ratio = 1 - self.prob
                # exclude non-flip
...
class SeqPad(Pad):
    ...
    Please refer to `mmdet.datasets.TRANSFORMS.transforms.py:Pad` for detailed
    docstring.
    """
